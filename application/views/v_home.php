...
<section class="py-5 bg-white border-bottom" style="overflow-x: hidden;">
    <div class="container">
        <div class="text-center mb-5">
        ...
        <?php 
        // Membagi total partner menjadi 2 baris agar tampilan rapi
        $jumlah_partner = count($partner);
        $per_baris = ceil($jumlah_partner / 2);
        $baris = array_chunk($partner, $per_baris);
        ?>

        <div class="partner-slider mb-4">
            <div class="partner-track-right" style="width: calc(200px * <?= count($baris[0]) * 2; ?>);">
                <?php foreach($baris[0] as $p): ?>
                    <div class="partner-item">
                        <img src="<?= base_url('assets/images/partner/' . $p['logo']); ?>" alt="<?= $p['nama_partner']; ?>">
                    </div>
                <?php endforeach; ?>
                <?php foreach($baris[0] as $p): ?>
                    <div class="partner-item">
        ...
        <div class="partner-slider">
            <div class="partner-track-left" style="width: calc(200px * <?= count($baris[1]) * 2; ?>);">
                <?php foreach($baris[1] as $p): ?>
                    <div class="partner-item">
                        <img src="<?= base_url('assets/images/partner/' . $p['logo']); ?>" alt="<?= $p['nama_partner']; ?>">
                    </div>
                <?php endforeach; ?>
    ...
    <style>
        .partner-slider { width: 100%; overflow: hidden; position: relative; }
        .partner-item { width: 200px; height: 100px; padding: 0 20px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; }
        .partner-item img { height: 100px; width: 180px; object-fit: contain; opacity: 0.7; transition: 0.3s; }
