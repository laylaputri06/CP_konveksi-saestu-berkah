    ...
    public function produk($kategori = null) // Ubah default menjadi null
{
    ...
    public function halaman()
    {
        ...
        $data['partner']    = $this->M_partner->get_all_partner();
   ...
   public function update_konten_halaman() 
   {
        ...
        $this->M_dashboard->add_aktivitas("Mengedit konten pada Halaman $nama_hal");
    ...
    {
        $data['title'] = 'Manajemen Partner';
        $data['partner'] = $this->db->get('tb_partner')->result_array();
        $data['nama_admin'] = $this->session->userdata('nama'); 

        date_default_timezone_set('Asia/Jakarta');
        $data['tanggal'] = date('l, d F Y');
        // Memanggil file v_partner.php
    ...
    public function tambah_partner() 
    {
        ...
        $nama_bersih = str_replace(' ', '_', $this->input->post('nama_partner'));
        $config['file_name'] = $nama_bersih;
        ...
        if ($this->upload->do_upload('logo')) {
            ...
            $this->M_dashboard->add_aktivitas("Menambahkan partner: " . $data['nama_partner']);
        }
    ...
    public function update_partner() 
    {
        ...
        if ($this->upload->do_upload('logo')) {
            ...
            $old = $this->db->get_where('tb_partner', ['id_partner' => $id])->row();
            if ($old && file_exists('./assets/images/partner/' . $old->logo)) {
                unlink('./assets/images/partner/' . $old->logo);
            }
            $img = $this->upload->data();
            $data['logo'] = $img['file_name'];
        }

        $this->db->update('tb_partner', $data, ['id_partner' => $id]);
        $this->session->set_flashdata('success', 'Partner berhasil diupdate!');
        $this->M_dashboard->add_aktivitas("Berhasil mengedit data partner: " . $this->input->post('nama_partner'));
        redirect('admin/partner');
    }
